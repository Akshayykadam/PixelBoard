...
- Fixed Describe Fallback and Example Hallucination: Resolved an issue where using "Describe your edit" could randomly return the built-in few-shot template example ("Team, please be advised that the project is moving forward. The new deadline is October 15th. More details will be shared soon."). This was caused by multi-role prompt delimiter splitting conflicts, an unpopulated draft in the prompt controller, and missing draft completion triggers.
- Active Draft Recovery in Describe: When "Describe your edit" is invoked before selecting a style chip, the runtime now dynamically retrieves the user's active editor draft from the trigger info and passes it to the prompt generator, ensuring the model always receives the text to rewrite.
- Aligned Multi-Role Prompt Semantics: Set `writing_tools_v2_enable_multi_role_prompt = false` and `writing_tools_enable_prompt_role = false` to match official stock single-payload prompt behavior and avoid empty-message splitting errors that produced "Couldn't load suggestions".
- Prompt Completion Anchoring: Improved prompt adaptation to ensure prompts transition directly into output draft generation tags, preventing the remote model from repeating prompt footer instructions.
